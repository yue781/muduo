...
void EventLoop::loop()
{
    ...
    while (!quit_)
    {
        ...
        // mainLoop事先注册一个回调cb（需要subloop执行）wakeup subloop之后，执行之前注册的cb
        doPendingFunctors();
    }
...
void EventLoop::quit()
{
    ...
    if (!isInLoopThread()) // 2.在其他线程中调用quit，例如在一个subloop中，调用了mainloop的quit
    {
        wakeup();
    }
}
...
void EventLoop::queueInLoop(Functor cb)
{
    ...
    // “|| callingPendingFunctors_”的含义：当前loop正在执行回调，但是loop又有了新的回调
...
void EventLoop::doPendingFunctors() // 执行回调
{
    ...
    {   
        // 作用：从局部的functors取cb时，不会阻碍mainloop向subloop写入cb到pendingFunctors_中，可以并发操作
        std::unique_lock<std::mutex> lock(mutex_);
