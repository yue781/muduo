...
// 内核发送数据慢，而应用写得快，需要把待发送数据写入缓冲区，而且设置了水位回调
void TcpConnection::sendInLoop(const void *data, size_t len)
{
    ...
    {
        LOG_ERROR("disconnected, give up wrting!");
        return;
    }
    ...
    if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0)
    {
        ...
        if (nwrote >= 0)
        {
            ...
            {
                // 在这里将数据全部发送，就不用再给channel设置epollpout事件了
    ...
    /*
    说明当前这一次write，并没有把全部数据发送出去，剩余的数据需要保存到缓冲区中，然后给channel
